...
main (int argc, char *argv[])
{
  ...
  topologyReader.SetFileName ("src/ndnSIM/examples/topologies/ndn-simple-with-l2tracer.txt");
  topologyReader.Read ();
  /****************************************************************************/
  // Install CCNx stack on all nodes
  ndn::StackHelper ccnxHelper;
  ccnxHelper.SetContentStore ("ns3::ndn::cs::Lru","MaxSize", "1000");
  ccnxHelper.SetPit ("ns3::ndn::pit::Lru","MaxSize","1000","PitEntryPruningTimout","10");
  ...
  boost::tuple< boost::shared_ptr<std::ostream>, std::list<Ptr<L2RateTracer> > >
   l2tracers = L2RateTracer::InstallAll ("l2-rate-tracer.txt", Seconds (0.5));
